...
//Done!
public class Schuhpflege extends Thread {
	@Override
	public void run() {
		...
		String Statement = "SELECT shop_item.description, shop_item.item_no FROM shop_item INNER JOIN shop_item_has_category "
				...
				+ "WHERE shop_item_has_category.category_line_no = 44960 ";
		String File = "Schuhpflege.csv";
		try {
		...
		try {
			entry = Golflaedchen.getProducts("https://www.golflaedchen.de/Schuhpflege/");

			entry.addAll(BestPreisGolf.getProducts("https://www.best-preis-golf.de/golfschuhe/schuhzubehoer/"));
			
			//nothing found in www.golfundguenstig.de
