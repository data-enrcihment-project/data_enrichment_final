...
public class Komplettsets extends Thread {
	...
	public void run() {
		...
		String Statement = "SELECT shop_item.description, shop_item.item_no FROM shop_item INNER JOIN shop_item_has_category "
				...
				+ "WHERE shop_item_has_category.category_line_no = 27500 ";
		String File = "Komplettsets.csv";
		try {
		...
		try {
			entry = Golflaedchen.getProducts("https://www.golflaedchen.de/Golfschlaeger-Golfset/");

			entry.addAll(BestPreisGolf.getProducts("https://www.best-preis-golf.de/golfschlaeger/komplettsets/"));

			entry.addAll(GolfUndGuenstig.getProducts("https://www.golfundguenstig.de/Golfschlaegersaetze/Komplettsaetze/"));
			
			entry.addAll(GolfUndGuenstig.getProducts("https://www.golfundguenstig.de/Golfschlaegersaetze/Komplettsaetze/2/"));
